...
static void app_network_print_qr(const char *name, const char *pop, const char *transport)
{
    ...
    ESP_LOGI(TAG, "If QR code is not visible, copy paste the below URL in a browser.\n%s?data=%s", QRCODE_BASE_URL, encoded_payload);
...
bool app_network_get_provisioned(){
    bool wifi_provisioned = false;
    /* Let's find out if the device is provisioned */
    #if RMAKER_USING_NETWORK_PROV
        network_prov_mgr_is_wifi_provisioned(&wifi_provisioned);
    #else
        esp_err_t err = wifi_prov_mgr_is_provisioned(&wifi_provisioned);
        if (err != ESP_OK) {
            ESP_LOGE(TAG, "wifi_prov_mgr_is_provisioned error.");
        }
    #endif
